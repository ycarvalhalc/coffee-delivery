import { ShoppingCart } from "phosphor-react"
import { useContext, useState, FormEvent, ChangeEvent } from "react"
import { CartContext } from "../../../context/CartContext"
import { QuantityButton, BuyButton } from "./styles"
import { Product } from "../../../reducers/cart/reducer"

interface ProductActionFormProps {
  product: Product
}

export function ProductActionForm({ product }: ProductActionFormProps) {
  const { AddProductToCart} = useContext(CartContext)
  const [quantity, setQuantity] = useState(1)

  function handleAddToCart(event: FormEvent) {
    event.preventDefault()

    AddProductToCart({
      product,
      quantity
    })
    setQuantity(1)
  }

  function handleChangeQuantity(event: ChangeEvent<HTMLInputElement>) {
    setQuantity(parseInt(event.target.value))
  }

  function handleRemoveOneUnity(event: FormEvent) {
    event.preventDefault()
    setQuantity((state) => --state)
  }

  function handleAddOneUnity(event: FormEvent) {
    event.preventDefault()
    setQuantity((state) => ++state)
  }
  
  return (
    <form onSubmit={(event) => handleAddToCart(event)}>
      <div>
        <QuantityButton 
          disabled={quantity === 1}
          onClick={(event) => handleRemoveOneUnity(event)}
        >
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" clipRule="evenodd" d="M1.53125 7C1.53125 6.63756 1.82506 6.34375 2.1875 6.34375H11.8125C12.1749 6.34375 12.4688 6.63756 12.4688 7C12.4688 7.36244 12.1749 7.65625 11.8125 7.65625H2.1875C1.82506 7.65625 1.53125 7.36244 1.53125 7Z" fill="#8047F8"/>
          </svg>
        </QuantityButton>

        <input 
          type="number" 
          min={1} 
          max={99} 
          value={quantity} 
          onChange={(event) => handleChangeQuantity(event)}
        />

        <QuantityButton 
          disabled={quantity === 99}
          onClick={(event) => handleAddOneUnity(event)}
        >
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" clipRule="evenodd" d="M1.53125 7C1.53125 6.63756 1.82506 6.34375 2.1875 6.34375H11.8125C12.1749 6.34375 12.4688 6.63756 12.4688 7C12.4688 7.36244 12.1749 7.65625 11.8125 7.65625H2.1875C1.82506 7.65625 1.53125 7.36244 1.53125 7Z" fill="#8047F8"/>
            <path fillRule="evenodd" clipRule="evenodd" d="M7 1.53125C7.36244 1.53125 7.65625 1.82506 7.65625 2.1875V11.8125C7.65625 12.1749 7.36244 12.4688 7 12.4688C6.63756 12.4688 6.34375 12.1749 6.34375 11.8125V2.1875C6.34375 1.82506 6.63756 1.53125 7 1.53125Z" fill="#8047F8"/>
          </svg>
        </QuantityButton>
      </div>
      <BuyButton>
        <ShoppingCart size={22} weight="fill" />
      </BuyButton>
    </form>
  )
}